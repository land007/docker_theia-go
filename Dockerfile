...
RUN apt-get update && apt-get upgrade -y && \
	...
	chown -R theia:theia /node_modules && chown -R theia:theia /usr/local/lib/node_modules && chown -R theia:theia /usr/local/bin/
ADD *.sh /
RUN sed -i 's/\r$//' /*.sh ; chmod +x /*.sh && \
	echo $(date "+%Y-%m-%d_%H:%M:%S") >> /.image_times && \
	echo $(date "+%Y-%m-%d_%H:%M:%S") > /.image_time && \
	echo "land007/theia-go" >> /.image_names && \
	echo "land007/theia-go" > /.image_name
...
#	echo 'node /home/theia/src-gen/backend/main.js /home/project --hostname=0.0.0.0 --startup-timeout=-1 --inspect=0.0.0.0:9229' >> /start.sh
	echo 'node /home/theia/src-gen/backend/main.js /home/project --hostname=0.0.0.0' >> /start.sh

USER theia
CMD /task.sh ; /start.sh ; bash

#docker build -t land007/theia-go:latest .
#docker rm -f theia-go; docker run -it --privileged -p 15050:5050 --expose 9229 -p 19229:9229 -v "$(pwd):/home/project:cached" -e "username=land007" -e "password=fcea920f7412b5da7be0cf42b8c93759" --name theia-go land007/theia-go:latest
#docker rm -f theia-go; docker run -it --privileged --expose 1080 -p 1080:1080 --expose 3000 -p 3000:3000 -p 15050:5050 --expose 9229 -p 19229:9229 -v "$(pwd):/home/project:cached" -e "username=land007" -e "password=fcea920f7412b5da7be0cf42b8c93759" --name theia-go land007/theia-go:latest
